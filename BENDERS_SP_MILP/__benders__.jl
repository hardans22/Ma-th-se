...
function benders_decomp(env, instance_data, sp_method, output_file, nbr_thread, silent, time_limit)
    ...
    set_optimizer_attribute(master_model, "FeasibilityTol", 1e-9)
    set_optimizer_attribute(master_model, "LogFile", output_file)
    
    #set_optimizer_attribute(master_model, "ZeroHalfCuts", 0)
    #= set_optimizer_attribute(master_model, "Cuts", 0)
    set_optimizer_attribute(master_model, "Heuristics", 0.0)
    set_optimizer_attribute(master_model, "RINS", 0)
    set_optimizer_attribute(master_model, "PumpPasses", 0)
    set_optimizer_attribute(master_model, "ZeroObjNodes", 0)
     =#
    #set_optimizer_attribute(master_model, "MIPFocus", 3)
    ...
    nbcp_fais, nbcp_opti, nbcp_cover, nbr_iter = 0, 0, 0, 0                                
    all_sptime = 0
    best_sp_obj = 0
    ...
    function my_callback(cb_data)
        ...
        xx_val = Dict{VariableRef, Float64}(x[a] => callback_value(cb_data, x[a]) for a in A)
        ...
        #= critical_subpath = find_critical_subpath(instance_data, aircraft_paths)
        best_subpath = find_best_subpath(result, instance_data, critical_subpath)
         =#
        #print_u(result.u, aircraft_paths)

        #println("\n\n")

        #print_u(result_1.u, aircraft_paths)

        #= cmp_rslt = compare_solution(result, result_1, V, L_M)
    
        if !cmp_rslt                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      
            println("\nObj du MILP = $(result.obj)")
            println("Time du MILP = $(result.time)")
            println("Obj du SP = $(result_1.obj)")
            println("Time du SP = $(result_1.time)")
        end 
        =#
        ...
        if result.status == "OPTIMAL"
            ...
            print_x_y(x_val, y_val, rho_val, A, L_M, d, H_aircraft)
             =#
            #= println("BEST_SUBPATH")
            println(best_subpath)
            println("SUB_PATH")
            println(sub_paths)
            =#
            #= for ac in keys(sub_paths)
                if best_subpath[ac].len_path !== sub_paths[ac][2]
                    println()
                    print_x_y(x_val, y_val, rho_val, A, L_M, d, H_aircraft)

                    println(ac)
                    println("============= BEST SUBPATH =============")
                    println(best_subpath[ac])
                    println("============= CE QUE J'AI CONSTRUIS =============")
                    println(sub_paths[ac])
                end  
            end 
         =#
         ...
         =#
            ...
            if gap > 1e-6
                ...
                benders_cuts[nbr_iter] = cut_list

                #irreductible_set(sub_paths, instance_data, sp_obj, d)
    ...
    println("BEST SP OBJ = ", best_sp_obj)
