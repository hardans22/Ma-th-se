...
for json_file in "$JSON_DIR"/*.json; do
    for preprocess in false; do
        for tk_option in MIN MAX; do 
            for fd_option in MIN MAX; do 
                if [ $SLURM_ARRAY_TASK_ID -eq $i ]
                then
                    module load julia
                    module load gurobi
                    julia /home/danhar/projects/def-mattgru/danhar/Code/SENSITIVE_EXP/main_exp_nibi2.jl "$json_file" $preprocess $tk_option $fd_option 
                fi
                ((i = $i + 1))
            done 
        done 
