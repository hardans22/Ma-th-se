...
function benders_decomp(env, instance_data, output_file, nbr_thread, silent, time_limit)
    ...
    L_mtn_stations = node_sets.mtn_nodes_stn
    V = graph.nodes
    ...
    @constraint(master_model, mp_c4[k in H_aircraft], sum(y[j,k] for j in L_M) <= 1)
    @constraint(master_model, mp_c5[j in L_M, k in setdiff(a_nodes, H_aircraft)], sum(y[j,k] for j in L_M) == 0)
    ...
    #= prep1_time = @elapsed begin
        ...
        all_cover_set = find_cover_set_2(H_aircraft, d, max_flt, succ_A, 100)
    ...
    spdual_model = build_spdual(env, instance_data)
    ...
    # Liste des nomtn_stations de variables à mapper
    ...
    function my_callback(cb_data)
        ...
        result = solve_spdual(x_val, y_val, instance_data, spdual_model)

        spd_obj = result.obj
        #= n_solutions = result_count(spdual_model)
        println("Nombre de solutions trouvées : $n_solutions")
        =#
        #print_information(result, instance_data, A_theta2, A_theta3, eq_nodes)
        ...
        if result.status == MOI.OPTIMAL
          ...
          =#
            ...
            for j in L_M
             ...
             =#
            #result_aux = solve_aux_spdual(x_val, y_val, x_core, y_core, instance_data, aux_spdual, spd_obj)
            ...
            if gap > 1e-6
               ...
               # Affichage complet
                aff_expr = coupe.func
                println("=== TOUS LES TERMES ($(length(aff_expr.terms)) termes) ===")
                for (var, coef) in aff_expr.terms
                    println("  $coef * $(name(var))")
                end
                println("Constante : $(aff_expr.constant)")
                println("RHS : $(coupe.set.upper)")
                println("Somme coeff (hors constante) : $(sum(values(aff_expr.terms)))")
                println("Somme + constante : $(sum(values(aff_expr.terms)) + aff_expr.constant)")
                print_x_y(x_val, y_val, A, L_M, d, ["N121", "N122", "N123"])
                #= if nbcp_opti == 5
                    print_information(result, instance_data, A_theta2, A_theta3, eq_nodes)
                    print_x_y(x_val, y_val, A, L_M, d, a_nodes)
                    println()
                 ...
                 =#
                 y_list = y_neighborhood(x_val, y_val, H_aircraft, A, L_M, A_M)
                #println("NOMBRE DE SOLUTIONS DANS LE VOISINAGE DE Y : ", length(y_list))
                 for y_sol in y_list
                    result = solve_spdual(x_val, y_sol, instance_data, spdual_model)
                    acd_sptime += result.time
 ...
 =#
            ...
            #println(length(x_neighbors))
             for x_new in x_neighbors
                result = solve_spdual(x_new, y_val, instance_data, spdual_model)
                if result.status == MOI.DUAL_INFEASIBLE
                    #nbcp_fais += 1   
                    acd_sptime += result.time
                    coupe = build_fais_cut(result, x, y, max_flt, d, f, A_M, A_theta2, A_theta3, L_M, a_nodes, V)
            ...
            all_cover_set = find_cover_set(A, succ_A, x_val, y_val, H_aircraft, d, max_flt, L_M)  
    ...
    for mtn_stations in remaining_stations
        if !isempty(intersect(Set(L_mtn_stations[mtn_stations]), active_j))
            push!(mtn_stations_used, mtn_stations)
        end
    end
