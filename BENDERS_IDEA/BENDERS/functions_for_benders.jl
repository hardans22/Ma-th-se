...
function solve_aux_spdual(x_val, y_val, core_x_val, core_y_val, instance_data, aux_spdual, sp_obj)
    graph = instance_data.graph
    node_sets = graph.node_sets
    arc_sets = graph.arc_sets
    fl_data = instance_data.fl_data
...
end A_M, L_M = instance["A_M"], instance["L_M"]
...
function print_information(spd_result, instance_data, A_theta2, A_theta3, eq_nodes)
    graph = instance_data.graph
    node_sets = graph.node_sets
    arc_sets = graph.arc_sets
    fl_data = instance_data.fl_data
