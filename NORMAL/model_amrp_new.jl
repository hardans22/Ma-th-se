...
function model_amrp(env, instance_file, output_file, nbr_thread, silent, preprocess, time_limit, FH, FC, DY, MTN_CAP, tk_option, fd_option)
    ...
    @constraint(model, sum(y[j] for j in L_M) <= ac_critique)
