...
function solve_model(model, instance_data, fix_set, mip_set, relax_set, sx, rf_or_fo)
    ...
    # IMPORTANT: D'abord tout unfixer et tout rendre binaire (état par défaut)
    for arc in A
        if is_fixed(x[arc])
            unfix(x[arc])
        end
    ...
    # Ensuite fixer les variables du fix_set
    for day in fix_set
        arcs_set = [arc for (arc, v) in arc_day if v == day]
        for arc in arcs_set
            fix(x[arc], sx[arc]; force = true)
    ...
    # Les variables du mip_set restent binaires (déjà fait au début)
    # Pas besoin de refaire set_binary pour mip_set
    
    # Pour le relax_set, enlever la contrainte binaire si nécessaire
    if rf_or_fo == "RF"
        for day in relax_set    
            arcs_set = [arc for (arc, v) in arc_day if v == day] 
            for arc in arcs_set
...
function fix_and_optimize(model, sx, instance_data, output_file, size_day, overlap)
    graph = instance_data.graph
    node_sets = graph.node_sets
    other_data = instance_data.other_data
    ...
    while curseur + overlap <= end_day 
        ...
        #println("\nCurrent curseur at day ", curseur)
        mip_set = [i for i in curseur:min(curseur + overlap, end_day)]
        fix_set = vcat([i for i in start_day:curseur - 1], [i for i in curseur+overlap+1:end_day]) 
    end
    timeElapsed = round(time() - begin_time, digits = 4) 
    sv, sw = Dict(j => 0 for j in V), Dict(j => 0 for j in V)
    s_lambda, s_phi = Dict(j => 0 for j in L_M), Dict(j => 0 for j in L_M)
