...
function combine_all_results(init_path, output_without, output_with, output_benders)
    ...
    for first_fold in ["A_MTN_2_cst_y/"]
        ...
        # Traiter les fichiers "with"
        benders_files = sort(glob("*benders*.xlsx", current_path))
        for file in benders_files
            println("Processing with file: ", file)
            df = DataFrame(XLSX.readtable(file, "Sheet1"))
            
            # Ajouter les colonnes d'identification au début
            df_benders_labels = DataFrame()
            df_benders_labels.AC = fill(mw_label, nrow(df))
            
            # Ajouter toutes les autres colonnes
            for col in names(df)
                df_benders_labels[!, col] = df[!, col]
            end
            
            push!(all_benders_data, df_benders_labels)
        end =#
    end
    ...
    if !isempty(all_benders_data)
        combined_benders = vcat(all_benders_data..., cols=:union)
        XLSX.writetable(output_benders, combined_benders, overwrite=true)
        println("Fichier 'benders' créé: ", output_benders)
        println("Nombre total de lignes 'benders': ", nrow(combined_benders))
    end =#
end
...
output_benders = init_path * "FINAL_all_results_benders.xlsx"

combine_all_results(init_path, output_without, output_with, output_benders)