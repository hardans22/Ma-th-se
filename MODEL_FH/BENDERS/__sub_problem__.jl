...
function solve_sp(sp_model, x_val, L_M, V)
...
function irreductible_path(ac_path, result, instance_data)
    graph      = instance_data.graph
    fl_data    = instance_data.fl_data
    A          = graph.arcs
    d          = fl_data.d
...
#= function check_cuts(benders_cuts, x_val, z_val; tol=1e-12)
    println("="^50)
    println("Vérification des Benders cuts")
    println("="^50)
    ...
    for (key, cuts_list) in benders_cuts
        for cut in cuts_list
            ...
            for (coef, var) in linear_terms(expr)
                if var in keys(x_val)
                    x_sum += coef * x_val[var]
                else
                    z_coef = coef
                    z_var_name = var
                end
            ...
            # Sécurité : ignorer les coupes sans terme Z
            if isnothing(z_var_name) || !(z_var_name in keys(z_val))
                println("⚠️  Coupe $key sans variable Z reconnue, ignorée.")
                continue
            end

            effective_rhs = cut.set.upper - x_sum
            z_val_current = z_val[z_var_name]
            slack = effective_rhs - z_coef * z_val_current

            status = slack >= -tol ? "✅ satisfait" : "❌ violé"

            if slack < -tol
                println("Coupe itération $key : -Z[$z_var_name] ≤ $(round(effective_rhs, digits=4)) | -Z = $(round(-z_val_current, digits=4)) | slack = $(round(slack, digits=4)) → $status")
                println(cut)
            end
        end
    end
