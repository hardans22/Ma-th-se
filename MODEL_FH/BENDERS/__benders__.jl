...
function benders_decomp_ind(env, instance_data, sp_method, cut_type, output_file, nbr_thread, silent, time_limit)
    #= if graph_reduc
        instance_data = graph_reduction(instance_data)
    end
    =#
        ...
        if sp_status == "OPTIMAL"
            ...
            if gap > 1e-6
                ...
                if obj_Z < best_obj
                    ...
                    for ac in a_nodes
                        u_ac, y_ac, rho_ac = all_result[ac].u, all_result[ac].y, all_result[ac].rho
                        for i in keys(u_ac)
                            u_val[i] = u_ac[i]
                        end
                        for j in keys(y_ac)
                            y_val[j], rho_val[j] = y_ac[j], rho_ac[j]
                        end
                    end
                    best_obj, best_x, best_y, best_u, best_rho = obj_Z, copy(x_val), copy(y_val), copy(u_val), copy(rho_val)
                end
