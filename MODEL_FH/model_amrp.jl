...
function model_amrp_lp(env, instance_data, nbr_thread, silent, graph_reduc)
    if graph_reduc
    ...
    # ===================== Decision variables =====================
    @variable(model, 0 <= x[k in A] <= 1)                          #Arc (i,j) selected
    @variable(model, 0 <= y[j in L_M] <= 1)                        #1 if a maintenance is performed at node j 
    ...
    if status == MOI.OPTIMAL
        ...
        other_info = Dict("status" => status, "time" => time,)
...
function model_amrp_with_warm_start(env, instance_data, x_start, y_start, obj_start, output_file, nbr_thread, silent, graph_reduc, time_limit)
    if graph_reduc
        instance_data = graph_reduction(instance_data)
    end
    ...
    write_to_file(model, "mon_model.lp") 

    # ===================== WARM START =====================

    for arc in A
        set_start_value(x[arc], x_start[arc])
    end

    for j in L_M
        set_start_value(y[j], y_start[j])
    end

    #set_optimizer_attribute(model, "Cutoff", obj_start)
    set_optimizer_attribute(model, "ImproveStartTime", 10)  # 300 secondes sans amélioration
