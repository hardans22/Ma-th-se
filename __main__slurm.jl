...
#= nbr_ac = parse(Int, ARGS[1])
nbr_day = parse(Int, ARGS[2])
preprocess = parse(Bool, ARGS[3]) =#
...
instances_list = sort(glob("*_"*string(nbr_ac)*"A_"*string(nbr_day)*"D_*.json", "./instances_json/"))
for preprocess in [false, true]
    ...
    for instance in instances_list
        ...
        if preprocess
            path_file = Outputs_fold*"result_"*string(nbr_ac)*"A_"*string(nbr_day)*"D_with_prep.txt"
        else 
            path_file = Outputs_fold*"result_"*string(nbr_ac)*"A_"*string(nbr_day)*"D_without_prep.txt"
        end
    ...
    if preprocess
        XLSX.writetable(Outputs_fold*"result_"*string(nbr_ac)*"A_"*string(nbr_day)*"D_with_prep.xlsx", dataframe, overwrite=true)
    else
        XLSX.writetable(Outputs_fold*"result_"*string(nbr_ac)*"A_"*string(nbr_day)*"D_without_prep.xlsx", dataframe, overwrite=true)
    end
end