...
function print_solution(solution, output_file, silent=false)
    ...
    instance_data = solution["instance_data"]
   ...
   #=  write_both("Nombre de vols satisfaits: $(length(fl_satisfied))")
    ...
    for aircraft in a_nodes
        ...
        #write_both("")
        for j in L_M
            if sy[j,aircraft] >= 0.9
                aircraft_mtn[aircraft] = j
            end
        end
        if haskey(aircraft_mtn, aircraft)
            println("\n🛠️ Maintenance at nodes: $(aircraft_mtn[aircraft]) by aircraft $aircraft | ρ=$(rho[aircraft_mtn[aircraft]]); λ=$(lambda[aircraft_mtn[aircraft]]); φ=$(phi[aircraft_mtn[aircraft]]) ")
        end
